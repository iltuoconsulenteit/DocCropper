...
# stop running app so license files can be written
if [ -f scripts/stop_DocCropper.command ]; then
  log "Stopping running DocCropper"
  scripts/stop_DocCropper.command >/dev/null 2>&1 || true
fi
